...
/// @title DeployV8ArbSepolia
/// @notice Deploy FheatherXv8 hooks with all 6 pool types on Arbitrum Sepolia
/// @dev Run with: source .env && forge script script/DeployV8ArbSepolia.s.sol:DeployV8ArbSepolia --rpc-url $ARB_SEPOLIA_RPC --broadcast -vvv
///
/// This script:
...
contract DeployV8ArbSepolia is Script {
    using stdJson for string;
    using PoolIdLibrary for PoolKey;

    // ============ Arbitrum Sepolia Addresses ============
    // Uniswap v4 (from v6-arb-sepolia.json deployment)
    ...
    // Existing Faucet Tokens from v6 deployment (ERC20)
    ...
    // FHERC20 tokens from v6 deployment
    ...
    string constant DEPLOYMENTS_PATH = "deployments/v8-arb-sepolia.json";
    ...
    function _seedLiquidity(address deployer) internal {
        ...
        if (!canSeedLiquidity) {
            ...
            console.log("Run: cast send <token> 'faucet()' --rpc-url $ARB_SEPOLIA_RPC --private-key $PRIVATE_KEY");
            return;
        }
    ...
    function _printSummary() internal view {
        ...
        console.log("1. Update frontend addresses in addresses.ts for chain 421614");
        console.log("2. Update poolStore with new v8 hooks");
