...
/// @title FheatherX v7 - Better Order Matching with 1× AMM Invariant
/// @author FheatherX Team
/// @notice A Uniswap v4 Hook combining encrypted AMM liquidity with improved limit order matching
/// @dev Key improvements over v6:
///      1. 1× AMM Update: Single AMM execution per swap (major gas savings)
///      2. Opposing Limit Matching: Limits matched at tick prices before AMM
///      3. Word-Level Bitmap: O(1) tick scanning using bit manipulation
///      4. Momentum Closure: Binary search for trigger order activation
///      5. Virtual Slicing: Fair pro-rata allocation to triggered orders
contract FheatherXv7 is BaseHook, Pausable, Ownable {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //                    LIMIT ORDER MATCHING (v7: New Pipeline)
    // ═══════════════════════════════════════════════════════════════════════

    /// @notice Process orders and execute swap in unified pipeline
    ...
    /// @return userOutputFromAmm Output user receives from AMM
    function _processOrdersAndSwap(
        ...
        // Simple tick estimation: cap at ±120 ticks (2 buckets)
        int24 toTick;
        if (r.reserve0 > 0 && r.reserve1 > 0) {
            uint256 impact = FullMath.mulDiv(userInputPlaintext, 100, zeroForOne ? r.reserve0 : r.reserve1);
            int24 delta = int24(int256(impact > 120 ? 120 : impact));
            toTick = zeroForOne ? prevTick - delta : prevTick + delta;
        } else {
            toTick = prevTick;
        }

        // Match opposing limits (fills at tick price, no AMM)
        euint128 remainderEnc;
        (remainderEnc, userOutputFromLimits) = _matchOpposingLimits(
            poolId, zeroForOne, userInputEnc, prevTick, toTick
        );
        FHE.allowThis(userOutputFromLimits);
        FHE.allowThis(remainderEnc);

        // Execute AMM with remainder (1x AMM update)
        ebool direction = FHE.asEbool(zeroForOne);
        userOutputFromAmm = _executeSwapMathForPool(poolId, direction, remainderEnc);
        FHE.allowThis(userOutputFromAmm);

        lastProcessedTick[poolId] = toTick;
        return (userOutputFromLimits, userOutputFromAmm);
    }

    /// @notice Match user's input against closest opposing limit order
    /// @dev Single bucket matching for gas/size optimization
    function _matchOpposingLimits(
        ...
        (int24 nextTick, bool found) = TickBitmapLib.findNextInitializedTick(
            bitmap, fromTick, TICK_SPACING, toTick < fromTick, 2
        );

        if (!found) return (userInputEnc, ENC_ZERO);

        return _fillOpposingBucket(poolId, nextTick, opposingSide, userInputEnc, zeroForOne);
    }
    ...
    function _fillOpposingBucket(
        ...
        euint128 encTickPrice = FHE.asEuint128(uint128(_calculateTickPrice(tick)));
        FHE.allowThis(encTickPrice);

        // Compute bucket capacity in user's input units
        ...
        _updateBucketOnFill(bucket, outputToUser, fill);
        ...
        // Event emission moved to caller for gas optimization
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //                   v7: DIRECT SWAP (New Pipeline)
    // ═══════════════════════════════════════════════════════════════════════

    /// @notice Execute a swap for a specific pool using new order matching pipeline
    /// @dev v7: Orders are matched BEFORE AMM, then single AMM execution
    function swapForPool(
        ...
        // 3. Process orders and execute AMM in unified pipeline
        (euint128 outputFromLimits, euint128 outputFromAmm) = _processOrdersAndSwap(
            poolId, zeroForOne, encAmountIn, amountIn
        );

        // 4. Calculate total output (use plaintext estimate for now)
        // The encrypted outputs are tracked internally; we use estimate for transfers
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //                    DIRECT ENCRYPTED SWAP (v7)
    // ═══════════════════════════════════════════════════════════════════════

    /// @notice Fully encrypted swap with direction hint for order matching
    /// @dev v7: Uses directionHint for order matching, encrypted direction for AMM
    /// @param directionHint Plaintext hint for tick range search (privacy tradeoff)
    function swapEncrypted(
        ...
        // v7: Process orders using direction hint for tick search
        // Note: plaintext amountIn not available, use 0 (conservative)
        (euint128 outputFromLimits, euint128 outputFromAmm) = _processOrdersAndSwap(
            poolId, directionHint, amt, 0
        );

        // Total output
        amountOut = FHE.add(outputFromLimits, outputFromAmm);
        FHE.allowThis(amountOut);
    ...
    /// @notice Manually trigger reserve sync
    function trySyncReserves(PoolId poolId) external {
        _harvestResolvedDecrypts(poolId);
        // Note: Order processing now happens during swapForPool/swapEncrypted
    }
