...
### 5.4 OrderTypeSelector

```typescript
// src/components/orders/OrderTypeSelector.tsx

import { ORDER_TYPE_INFO, type OrderType } from '@/lib/orders';
import { cn } from '@/lib/utils';
...
const ORDER_TYPES: OrderType[] = ['limit-buy', 'limit-sell', 'stop-loss', 'take-profit'];
...
### 5.5 TransactionLink (NEW)

```typescript
// src/components/common/TransactionLink.tsx
...
export function TransactionLink({ hash, label, className }: TransactionLinkProps) {
  ...
  if (!url) {
    return <span className={className}>{shortenAddress(hash, 6)}</span>;
  }
  ...
  return (
    <a
      ...
      className={`text-phoenix-ember hover:underline ${className}`}
    >
...
export function GasEstimate({ estimate, isLoading, className }: GasEstimateProps) {
  if (isLoading) {
  ...
  return (
    <div className={`text-sm text-feather-white/60 ${className}`}>
